...
echo "$($PSQL "SELECT name 
                FROM teams LEFT JOIN games ON teams.team_id=games.winner_id
                WHERE year=2018 AND round='Final' ")" 

echo -e "\nList of teams who played in the 2014 'Eighth-Final' round:"

echo "$($PSQL "SELECT name 
                FROM games JOIN teams ON games.winner_id=teams.team_id OR games.opponent_id=teams.team_id
...
echo "$($PSQL "SELECT DISTINCT name  
                FROM games LEFT JOIN teams ON teams.team_id=games.winner_id
                ORDER BY name ")" 
